...
export const getEntriesByContentType = async (content_type, slug = null) => {
  ...
  try {
    ...
    if (contentfulClient) {
      let params = { content_type: content_type, include: 4 };

      if (slug) {
        params["fields.slug"] = slug;
      }

      console.log(`Fetching ${content_type} with params:`, params);
      ...
      console.log(`Contentful response for ${content_type}:`, {
        total: entries.total,
        itemsCount: entries.items?.length || 0,
        includes: entries.includes ? Object.keys(entries.includes) : [],
        includesCount: entries.includes?.Entry?.length || 0,
      });
